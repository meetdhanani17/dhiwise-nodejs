...
import { Description, Heading, StepFooter } from "../../../../components";
...
export const LibraryPreview = ({ setPanel }) => {
  ...
  const { applicationId, currentApplicationCode, modelList } = useSelector(
    ({ projects, models }) => ({
  ...
  const checkedDataJson = (isRelationTable = false) =>
    checkedRefData.current
      .map((checked) => {
        if (
          checked.original.ref &&
          isRelationTable &&
          checked.isSelected &&
          libraryModelList.find((model) => model._id === checked.original.ref)
            ?.name !== selectedModel.name &&
          modelList.findIndex(
            (model) =>
              model.name?.toLowerCase() ===
              libraryModelList
                .find((library) => library._id === checked.original.ref)
                ?.name?.toLowerCase()
          ) < 0
        ) {
        ...
        if (checked.isSelected && !checked.original.rowKey) {
          return {
            ...
            subRows: checked?.subRows
              ?.filter((sub) => sub.isSelected || sub.isSelected === undefined)
              .map((sub) => ({ ...sub.original, description: undefined })),
          };
        }
        return false;
      })
  ...
  const checkedIndexData = () => {
    ...
    checkedRefIndexes?.current?.forEach((checked) => {
      if (checked.isSelected && !checked.original.rowKey) {
        const isExist = selectedModel?.modelIndexes?.find(
          (x) => x.name === checked?.original?.name
        );
        if (isExist) {
          selected.push(isExist);
        }
      }
    });
    return selected;
  ...
  const handelSubmit = () => {
    ...
    const mapIndexes = isArray(checkedRefIndexes?.current)
      ? checkedIndexData()
      : selectedModel.modelIndexes;
    if (!isEmpty(relationTableData.current)) {
      attentionRef.current.handelOpen();
      return;
    }
    if (isEmpty(mapTemplate)) {
      addErrorToast(
        "You cannot create template without selecting any attributes. Kindly select the attributes and then proceed further."
      );
      return;
    }
    setSubmit();
        ...
        <div className="flex justify-center items-center h-full">
          <div className="text-center">
            ...
            <Description variant="subTitle">
              DhiWise offers a number of pre-defined schemas. Explore it,
      ...
      {!isEmpty(selectedModel) && (
        <>
          <div className="px-3 py-2 previewTop">
            ...
            <Description className="mt-1 xxl:w-9/12">
          ...
          >
            <TabList className={`${TabCSs.tabHead}`}>
              <Tab className={TabCSs.tabTitle}>
                Schema
                {Object.keys(selectedModel.schemaJson || {}).length > 0
                  ? `(${Object.keys(selectedModel.schemaJson).length})`
                  : ""}
              </Tab>
              <Tab className={TabCSs.tabTitle}>
                {HOOK_TAB_TITLE[currentApplicationCode]}
                {selectedModel?.hooks?.length > 0
                  ? `(${selectedModel.hooks.length})`
                  : ""}
              </Tab>
              <Tab className={TabCSs.tabTitle}>
                Indexing
                {selectedModel?.modelIndexes?.length > 0
                  ? `(${selectedModel.modelIndexes.length})`
                  : ""}
              </Tab>
            </TabList>
            <TabPanel>
              <PreviewTable
                ...
                setCheckedRefData={(checked) => {
                  checkedRefData.current = checked;
                }}
              />
            </TabPanel>``
            <TabPanel>
            ...
            <TabPanel>
              <Indexing
                ...
                setCheckedRefData={(checked) => {
                  checkedRefIndexes.current = checked;
                }}
              />
            </TabPanel>
          </Tabs>
          <div>
            <StepFooter
              ...
              backClick={() => {
                libraryDispatch({ type: "SetSelectionModel", payload: {} });
              }}
              Back="Cancel"
              Next="Use template"
          ...
          <Attention
            ...
            checkedDataJson={() =>
              tableToCodeViewParser({
                tableArr: checkedDataJson(false),
                isReturnJson: true,
                customModelList: libraryModelList,
                isExcludeArrayId: true,
              })
