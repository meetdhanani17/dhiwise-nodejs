...
const tableToJsonParser = (table) => {
  ...
  table.forEach((tb) => {
    ...
    a[tb.attribute] =
      tb.type !== NODE_DATA_TYPE.JSON
        ? tb.value
        : tableToJsonParser(tb.subRows);
  });
  return a;
};
export const ConstantView = React.memo(() => {
  ...
  const currentConstantData = constantList.find(
    (d) => d._id === selectedConstant
  );
  ...
  const handleUpdate = async (deleteJson) => {
    ...
    if (
      isEmpty(tableToCode) &&
      !deleteJson &&
      isEmpty(currentConstantData?.customJson)
    )
      return;
    setLoading();
    await updateRef.current?.handelConstantProperty(
      "customJson",
      tableToCode,
      !deleteJson
    );
  ...
  React.useEffect(() => {
    setCode(
      isDisable && typeof currentConstantData?.customJson !== "object"
        ? currentConstantData?.customJson
        : JSON.stringify(
            JSON.parse(JSON.stringify(currentConstantData?.customJson || {})),
            null,
            2
          )
    );
  ...
  return (
    <div className="flex flex-col w-full h-full">
      ...
      {isError ? (
        <Error isOpen={isError} error={jsonError} handleCancel={setHideError} />
      ) : null}
    </div>
  );
});
